...
export default function SortableCategoryCard({ cat, onOpen, onEdit, onDelete }: Props) {
  ...
  return (
    <div ref={setNodeRef} style={style} className="bg-dark-3 rounded-lg overflow-hidden flex flex-col group">
      {/* div instead of button avoids button-in-button hydration error */}
      <div
        onClick={onOpen}
        role="button"
        tabIndex={0}
        onKeyDown={(e) => e.key === 'Enter' && onOpen()}
        className="aspect-video relative bg-dark-2 overflow-hidden flex-shrink-0 w-full cursor-pointer"
      >
        {imgUrl ? (
