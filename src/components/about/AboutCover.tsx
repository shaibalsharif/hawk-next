...
import type { AboutCover } from '@/types'

const XEN_EASE: [number, number, number, number] = [0.76, 0, 0.24, 1]

interface Props {
  data: AboutCover | null
  onScrollClick?: () => void
}

export default function AboutCover({ data, onScrollClick }: Props) {
  ...
  return (
    <div className="flex items-center bg-dark-2 relative z-0">
      ...
      <div className="h-screen px-8 sm:w-full flex items-center bg-black/30">
        <div className="md:w-[55%] overflow-hidden">
          ...
          <motion.div
            className="text-xs sm:text-sm font-[500] flex gap-6 pt-8 uppercase"
